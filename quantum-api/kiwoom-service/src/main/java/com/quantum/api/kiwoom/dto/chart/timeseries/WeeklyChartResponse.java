...
/**
 * 키움증권 주식주봉차트조회요청 (ka10082) 응답 DTO
 */
@Data
@EqualsAndHashCode(callSuper = true)
@SuperBuilder
@Schema(description = "주식주봉차트조회 응답")
public class WeeklyChartResponse extends BaseChartResponse {
     ...
     * 주식주봉차트조회 데이터 리스트
     */
    @JsonProperty("stk_wk_pole_chart_qry")
    @Schema(description = "주봉 차트 데이터 목록")
    private List<WeeklyChartData> chartData;
    ...
    public List<CandleData> toCandleDataList() {
        ...
        return chartData.stream()
                .map(WeeklyChartData::toCandleData)
     ...
     * 첫 번째 (최신) 데이터 반환
     */
    public WeeklyChartData getLatestData() {
        if (isEmpty()) {
            return null;
        }
     ...
     * 마지막 (가장 오래된) 데이터 반환
     */
    public WeeklyChartData getOldestData() {
        if (isEmpty()) {
            return null;
        }
