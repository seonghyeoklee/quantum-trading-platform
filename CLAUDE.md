...
│   ├── dashboard.py           # 실시간 트레이딩 대시보드 (HTML + JS 폴링)
│   ├── kis/                   # KIS API 클라이언트
...
│   ├── report_theme.py         # 공통 HTML 리포트 테마 (다크 테마 CSS + wrap_html)
│   └── api/
│       └── routes.py          # API 엔드포인트 (매매 + 전략 전환 + 백테스트 + 저널 + 대시보드)
...
│   ├── test_dashboard.py      # 대시보드 HTML 구조 테스트 (14개)
...
브라우저에서 `http://localhost:8000/dashboard` 접속. 서버 사이드 데이터 주입 없이 JS가 기존 API를 폴링하여 화면을 채우는 구조.

### 구성

```
┌──────────────────────────────────────────────────┐
│  HEADER: "Quantum Trading Dashboard"  [갱신 시각] │
├───────────┬──────────┬──────────┬────────────────┤
│ 엔진 상태  │ 전략 정보 │ 예수금    │ 평가손익        │
├───────────┴──────────┴──────────┴────────────────┤
│  보유 포지션 테이블 (국내 + 해외)                    │
├──────────────────────────────────────────────────┤
│  당일 평가손익 추이 차트 (Chart.js line)             │
├────────────────────┬─────────────────────────────┤
│  최근 시그널 (20건)  │  최근 주문 (20건)             │
└────────────────────┴─────────────────────────────┘
```
...
| 엔드포인트 | 주기 | 용도 |
|-----------|------|------|
| `GET /trading/status` | 5초 | 엔진 상태, 시그널, 주문 |
| `GET /trading/positions` | 10초 | 포지션, 예수금, P&L 추이 축적 |
| `GET /trading/strategy` | 로드 시 1회 | 전략 타입/파라미터 표시 |

### 관련 파일

| 파일 | 역할 |
|------|------|
| `app/dashboard.py` | `build_dashboard_html()` — HTML 쉘 + CSS + JS 전체 |
| `app/report_theme.py` | `wrap_html()` — 다크 테마 공통 골격 |
| `app/api/routes.py` | `GET /dashboard` 엔드포인트 |
| `tests/test_dashboard.py` | HTML 구조 단위 테스트 (14개) |

### 특징

- **서버 사이드 데이터 주입 없음**: HTML 쉘만 반환, JS fetch로 채움
- **P&L 추이**: JS 배열에 시계열 축적 (최대 200포인트), 페이지 새로고침 시 초기화
- **에러 내성**: fetch 실패 시 마지막 성공 상태 유지
- **반응형**: 모바일 대응 (`@media` 브레이크포인트)
- **다크 테마**: 기존 리포트와 동일한 `report_theme.py` 공유
...
*Status: MVP - 국내/해외(미국)주식 모의투자 자동매매 + 시장 국면 기반 전략 선택 + 매매 저널 + 실시간 대시보드*
